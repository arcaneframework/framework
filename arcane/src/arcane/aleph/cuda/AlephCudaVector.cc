...
template <class T> CNC_Vector<T>::~CNC_Vector()
{
  deallocate();
}
...
template <class T> void CNC_Vector<T>::allocate(unsigned int size, unsigned int alignment)
{
  ...
  if (size != 0) {
    ...
    while (((unsigned long long)p) % alignment) {
      ++p;
    }
    data_ = (T*)p;
...
template <class T> unsigned int CNC_Vector<T>::size() const
{
  return size_;
}
...
template <class T> unsigned int CNC_Vector<T>::alignment() const
{
  return alignment_;
}
...
template <class T> void CNC_Vector<T>::clear()
{
  allocate(0);
}
...
template <class T> const T* CNC_Vector<T>::data() const
{
  return data_;
}
...
/** low-level access, for experts only. */
template <class T> T* CNC_Vector<T>::data()
{
  return data_;
}
