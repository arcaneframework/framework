...
init()
{
  ...
  // Update of UID and SDID variables
  ENUMERATE_CELL (icell, allCells())
...
initAlgebra()
{
  ...
  // We create our scheduling policy in place relative to the described topology
  if (m_aleph_kernel)
    delete m_aleph_kernel;
  ...
  // To test the specific PETSc initialization, add arguments
  ...
  m_aleph_params = new AlephParams(traceMng(),
                                   ...
                                   TypesSolver::PCG, // m_param_solver_method solution method
                                   -1, // m_param_gamma
                                   -1.0, // m_param_alpha
                                   ...
                                   0, // m_param_amg_coarsening_method: by default Sloop,
  ...
  // Calculate the global indices of the cell x cell matrix
  // And we prepare the vector indices once and for all
  m_cell_matrix_idx.fill(-1);
  {
    AlephInt idx = 0;
    // Very important, because we return to it during reconfiguration!!
  ...
  // Now, we will count the number of elements per row
  // We perform a scan as we will have to do in the loop
  //debug() << "\33[37m[AlephTestModule::initAlgebra] Counting the number of elements of local rows"<<"\33[0m";
...
postSolver(const Integer i)
{
  ...
  debug() << "\33[37m[AlephTestModule::postSolver] Filling the right-hand side"
          << "\33[0m";
