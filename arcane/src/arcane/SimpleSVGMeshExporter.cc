...
//-----------------------------------------------------------------------------
/*---------------------------------------------------------------------------*/
/* SimpleSVGMeshExporter.cc                                    (C) 2000-2021 */
/*                                                                           */
...
write(const CellGroup& cells)
{
  ...
  // Calcul le centre des mailles
  std::map<Int32,Real3> cells_center;
  ENUMERATE_CELL(icell,cells){
    ...
    if (nb_node>0){
      for( Integer i=0; i<nb_node; ++i )
        center += nodes_coord[cell.node(i)];
      center /= nb_node;
    }
    cells_center[cell.localId()] = center;
  }

  // Affiche pour chaque maille son contour et son uniqueId().
  ENUMERATE_CELL(icell,cells){
    Cell cell = *icell;
    Real3 cell_pos = cells_center[cell.localId()] * mul_value;
    Integer nb_node = cell.nbNode();
    ofile << "<path d='";
    for( Integer i=0; i<nb_node; ++i ){
  ...
  // avec son segment.
  {
    ...
    ENUMERATE_CELL(icell,cells){
      ...
      for( Integer i=0; i<nb_face; ++i ){
        ...
        Real3 face_coord = (node0_coord + node1_coord) / 2.0;
        ...
        Real3 cell_center = cells_center[cell.localId()] * mul_value;
