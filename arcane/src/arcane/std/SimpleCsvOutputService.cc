...
elem(String name_column, String name_row, bool update_last_pos)
{
  ...
  if(pos_x && pos_y) return elem(pos_x.value(), pos_y.value(), update_last_pos);
  return 0;
}
...
{
  std::optional<Integer> pos_y = m_name_rows.span().findFirst(name_row);
  if(pos_y) return row(pos_y.value());
...
{
  std::optional<Integer> pos_x = m_name_columns.span().findFirst(name_column);
  if(pos_x) return column(pos_x.value());
  return RealUniqueArray(0);
}
...
{
  std::optional<Integer> pos_y = m_name_rows.span().findFirst(name_row);
  if(pos_y) return sizeRow(pos_y.value());
  return 0;
}
...
{
  std::optional<Integer> pos_x = m_name_columns.span().findFirst(name_column);
  if(pos_x) return sizeColumn(pos_x.value());
  return 0;
}
...
writeFile(Integer only_proc)
{
  ...
  if(only_proc != -1 && mesh()->parallelMng()->commRank() != only_proc) return true;
  ...
  if((only_proc == -1 && only_one_proc) && mesh()->parallelMng()->commRank() != 0) return true;
