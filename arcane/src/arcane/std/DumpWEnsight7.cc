...
public:
  ...
  {
	public:
    EnsightPart() : m_type(-1), m_nb_node(0) {}
    EnsightPart(int type,Integer nb_node,const String& name)
			: m_type(type), m_nb_node(nb_node), m_name(name) {}
    EnsightPart(const EnsightPart& part) 
      : m_type(part.m_type), m_nb_node(part.m_nb_node), m_name(part.m_name)
      , m_items(part.m_items), m_reindex(part.m_reindex)
    {
      ;
    }
  public:
  ...
  struct GroupPartInfo
  {
	...
	public:
    GroupPartInfo() : m_group(0), m_part_id(0) {}
    GroupPartInfo(ItemGroup grp,Integer id, bool use_degenerated_hexa) : m_group(grp), m_part_id(id)
  ...
  private:
    void _init(bool use_degenerated_hexa)
      ...
      for(Integer i_type=ItemTypeMng::nbBuiltInItemType(); i_type<ItemTypeMng::nbBasicItemType(); ++i_type)
        {
          ...
          if (type_info->nbLocalNode() == type_info->nbLocalEdge())
            { // Polygone trouvé
      ...
      if (use_degenerated_hexa) {
        ...
        { Integer reindex[8] = { 1, 6, 5, 0, 2, 3, 4, 0 }; m_parts[m_parts.size()-1].setReindex(reindex); }
        m_parts.add(EnsightPart(IT_HemiHexa6,8,"hexa8")); // HemiHexa6
        { Integer reindex[8] = { 0, 1, 3, 5, 0, 2, 3, 4 }; m_parts[m_parts.size()-1].setReindex(reindex); }
        m_parts.add(EnsightPart(IT_HemiHexa5,8,"hexa8")); // HemiHexa5
        { Integer reindex[8] = { 0, 1, 3, 4, 0, 2, 3, 4 }; m_parts[m_parts.size()-1].setReindex(reindex); }
        m_parts.add(EnsightPart(IT_AntiWedgeLeft6,8,"hexa8")); // AntiWedgeLeft6
        { Integer reindex[8] = { 2, 0, 1, 2, 5, 3, 4, 4 }; m_parts[m_parts.size()-1].setReindex(reindex); }
        m_parts.add(EnsightPart(IT_AntiWedgeRight6,8,"hexa8")); // AntiWedgeRight6
        { Integer reindex[8] = { 2, 0, 1, 1, 5, 3, 4, 5 }; m_parts[m_parts.size()-1].setReindex(reindex); }
        m_parts.add(EnsightPart(IT_DiTetra5,8,"hexa8")); // DiTetra5
        { Integer reindex[8] = { 4, 4, 2, 3, 0, 1, 1, 3 }; m_parts[m_parts.size()-1].setReindex(reindex); }
      } else {
...
public:
  ...
  */
  ...
  {
	public:
    WriteBase(DumpWEnsight7& dw,SharedPtrT<GroupIndexTable> idx = SharedPtrT<GroupIndexTable>()) : m_dw(dw), m_idx(idx) {}
    WriteBase(const WriteBase& wb) : m_dw(wb.m_dw), m_idx(wb.m_idx) {}
    virtual ~WriteBase(){ ; }
	public:
		...
		: public WriteBase
  {
    ...
    WriteDouble(DumpWEnsight7& dw,ConstArrayView<FromType> ptr,SharedPtrT<GroupIndexTable> idx = SharedPtrT<GroupIndexTable>())
			: WriteBase(dw,idx), m_ptr(ptr){}
    WriteDouble(const WriteDouble& wd)
			: WriteBase(wd), m_ptr(wd.m_ptr) {}
  ...
  class WriteArrayDouble : public WriteBase
  {
   public:
    ...
    : WriteBase(dw,idx), m_ptr(ptr), m_idim2(idim2) {}
    WriteArrayDouble(const WriteArrayDouble& wd)
    : WriteBase(wd), m_ptr(wd.m_ptr), m_idim2(wd.m_idim2) {}
...
_writeFileHeader(ostream& o,bool write_c_binary)
{
  if (m_is_master){
    ...
    if (m_fileset_size!=0){
			ostringstream ostr;
			ostr << "BEGIN TIME STEP " << "# " << m_times.size();
...
public:
  DumpWEnsight7OutFile(DumpWEnsight7& dw,const String& filename,int outtype)
		: m_dw(dw), m_filename(filename), m_is_master(dw.isMasterProcessor()),
			m_is_parallel_output(dw.isParallelOutput()),
			m_stream(0), m_filestream(0)
	{
		if (m_is_master){
...
: public PostProcessorWriterBase
{
 public:
  ...
  : PostProcessorWriterBase(sbi), m_mesh(sbi.mesh()), m_writer(0)
    {
    }
...
public:
 ...
 public:
  ...
  : ArcaneEnsight7PostProcessorObject(sbi), m_mesh(sbi.mesh()), m_writer(0)
	{
	}
