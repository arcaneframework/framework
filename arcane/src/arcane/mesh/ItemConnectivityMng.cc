...
/*                                                                           */
/* Connectivity Manager for Entities.                                        */
/*---------------------------------------------------------------------------*/
/*---------------------------------------------------------------------------*/
...
notifyLocalIdChanged(IItemFamily* family, Int32ConstArrayView old_to_new_ids, Integer nb_item)
{
  ...
  for (Arcane::Integer i = 0; i < family_state.m_added_items.size(); ++i) {
    if (family_state.m_added_items[i] != NULL_ITEM_LOCAL_ID) {
      ...
      if (new_id < nb_item)
        family_state.m_added_items[i] = new_id; // OK valid item
      else
        family_state.m_added_items[i] = NULL_ITEM_LOCAL_ID; // KO invalid item. old_to_new has no null_item_lid. it has lid > nb_item. These lids will be removed in finish compact items...
    }
  }
