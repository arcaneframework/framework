 ...
 protected:
  ...
  bool isFrontCell(ItemLocalId source_item, ItemLocalId target_item)
  {
    ...
    for (Integer local_face_index = 0; local_face_index < cell_type_info->nbLocalFace() && face_index == -1; ++local_face_index) {
      ...
      if (local_face.nbNode() != face_nodes.size())
        continue;
      for (Integer local_node_index = 0; local_node_index < local_face.nbNode(); ++local_node_index)
        local_face_node_uids.add(cell_nodes[local_face.node(local_node_index)].uniqueId().asInt64());
      std::set<Int64> local_face_nodes_set(local_face_node_uids.begin(), local_face_node_uids.end());
      if (local_face_nodes_set == face_nodes_set)
        face_index = local_face_index; // we found the face in the cell
    }
    if (face_index == -1)
      throw FatalErrorException(String::format("Face {0} does not belong to Cell {1}. Cannot connect. Exiting."));
    if (mesh->dimension() == 1) // 1d mesh : back cell is local face 0, front is local face 1
      ...
      if (mesh_utils::reorderNodesOfFace(local_face_node_uids, ordered_face_node_ids))
        return true;
      else
        return false;
    }
