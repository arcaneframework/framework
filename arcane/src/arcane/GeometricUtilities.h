...
//-----------------------------------------------------------------------------
/*---------------------------------------------------------------------------*/
/* GeometricUtilities.h                                        (C) 2000-2018 */
/*                                                                           */
/* Fonctions utilitaires sur la géométrie.                                   */
/*---------------------------------------------------------------------------*/
#ifndef ARCANE_GEOMETRICUTILITIES_H
#define ARCANE_GEOMETRICUTILITIES_H
/*---------------------------------------------------------------------------*/
/*---------------------------------------------------------------------------*/
...
namespace GeometricUtilities
{
  ...
  {
   public:
    QuadMapping() : m_precision(1.0e-14)
    {
    }
   public:
    Real3 m_pos[4];
  ...
  public:
    ProjectionInfo(Real distance,int region,Real alpha,Real beta,Real3 aprojection)
      : m_distance(distance), m_region(region), m_alpha(alpha)
      , m_beta(beta), m_projection(aprojection) {}
    ProjectionInfo()
    : m_distance(FloatInfo<Real>::maxValue()), m_region(-1)
