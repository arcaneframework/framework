  ...
  */
  ...
  MeshMaterialModifier modifier(m_material_mng);
      ...
      for( Integer imat=0; imat<nb_mat; ++imat ){
        ...
        to_add[imat] = m_idx_selecter.syncSelectIf(m_queue,
        [=] ARCCORE_HOST_DEVICE (Int32 cid) -> bool {
          if(cells_info.isOwn(cid)) return false;
          CellLocalId c{cid};
        ...
        to_remove[imat] = m_idx_selecter.syncSelectIf(m_queue,
            [=] ARCCORE_HOST_DEVICE (Int32 cid) -> bool {
          if(cells_info.isOwn(cid)) return false;
          CellLocalId c{cid};
          AllEnvCell allenvcell{cell_converter[c]};
