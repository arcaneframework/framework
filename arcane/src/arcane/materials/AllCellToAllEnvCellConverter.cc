...
AllCellToAllEnvCell::
AllCellToAllEnvCell()
: m_mm(nullptr), m_alloc(nullptr), m_size(0), m_allcell_allenvcell(nullptr)
{
}
...
{
  if (m_size != m_mm->mesh()->allCells().itemFamily()->maxLocalId()+1) {
    ...
    for (Int32 i(0), n=ids.size(); i<n; ++i) {
      ...
      if (!m_allcell_allenvcell[lid].empty()) {
        m_allcell_allenvcell[i].~Span<ComponentItemLocalId>();
        m_alloc->deallocate(&(m_allcell_allenvcell[lid]));
      }
