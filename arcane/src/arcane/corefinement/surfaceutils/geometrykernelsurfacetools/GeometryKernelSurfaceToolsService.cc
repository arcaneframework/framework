...
#include "arcane/corefinement/surfaceutils/geometrykernelsurfacetools/GeometryKernelSurfaceToolsService.h"
...
void GeometryKernelSurfaceToolsService::
init()
{
  ;
}
    ...
    while (canContinue) {
      ...
      for (Integer iA = 0; iA < split_sizeA; ++iA) {
        ...
        if (jB[iA] != endB[iA])
          indexB = jB[iA]->indexB();
        if (indexB != voidIndex) {
          if (minIndexB == voidIndex)
            minIndexB = indexB;
          else
            minIndexB = math::min(minIndexB, indexB);
        }
      }
      ...
      struct ConvertFace
      {
        ConvertFace(const Array<Face>& face_array, const Integer voidIndex)
        : m_face_array(face_array)
        , m_void_index(voidIndex)
        {}
        Face operator()(Integer index) const
        {
          if (index == m_void_index)
            return Face();
          else
            return m_face_array[index];
        }
