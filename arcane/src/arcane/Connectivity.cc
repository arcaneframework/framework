...
freeze(IMesh * mesh)
{
  ...
  case 3:
    ...
    break;
  default:
    throw FatalErrorException(A_FUNCINFO,String::format("Mesh ({0}) dimension must be set before frezing connectivity (current={1})",mesh->name(),dim));
  }

  if (_hasConnectivity(m_connectivity(),CT_Dim2D) && hasConnectivity(m_connectivity(),CT_Dim3D))
    throw FatalErrorException(A_FUNCINFO,"A mesh cannot have both dimensions 2 and 3");
...
_checkFrozen() const
{
  if (m_connectivity() & CT_Frozen)
    throw FatalErrorException(A_FUNCINFO,"Cannot modify frozen connectivity");
}
