...
createWindow(Int32 my_rank_global, Integer nb_elem_local_proc, Integer sizeof_type, MpiParallelMng* mpi_parallel_mng)
{
  ...
  if (my_rank_local_proc == 0) {
    std::cout << "OK : " << my_rank_mpi << std::endl;
    ...
    std::cout << "OK2 : " << my_rank_mpi << std::endl;
    mpi_parallel_mng->barrier();
  ...
  auto* window_obj = new HybridMachineMemoryWindowBase(my_rank_mpi, my_rank_local_proc, m_nb_rank_local_proc, m_ranks, sizeof_type, m_nb_elem, m_sum_nb_elem, m_window, m_barrier);
  m_barrier->wait();
...
_buildMachineRanksArray(Mpi::MpiMachineMemoryWindowBaseCreator* mpi_window_creator)
{
  ConstArrayView<Int32> mpi_ranks(mpi_window_creator->machineRanks());
  m_ranks.resize(mpi_ranks.size() * m_nb_rank_local_proc);

  Integer iter = 0;
  for (Int32 mpi_rank : mpi_ranks) {
    for (Integer thread_rank = 0; thread_rank < m_nb_rank_local_proc; ++thread_rank) {
      m_ranks[iter++] = thread_rank + m_nb_rank_local_proc * mpi_rank;
    }
  }
}
