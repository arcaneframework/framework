...
_node2DoFConnectivity()
{
  ...
  ENUMERATE_NODE (inode, allNodes()) {
    ...
    if (connected_dof.owner() != inode->owner())
      fatal() << "Error in node to dof connectivity ghost policy";
...
_cell2DoFsConnectivity()
{
  ...
  if (!_checkIsSame(&cell2dofs, &cell2dofs_2))
    fatal() << "Error in connectivity construction from ItemArrayProperty";
}
...
_Face2DoFsMultiConnectivity()
{
  ...
  ENUMERATE_FACE (iface, ownFaces()) {
    ...
    for (Integer i = 0; i < nb_dof_on_face; ++i)
      uids.add(mesh::DoFUids::uid(max_dof_uid, max_face_uid, iface->uniqueId().asInt64(), i));
  }
  ...
  if (!_checkIsSame(&face2dofs, &face2dofs_2))
    fatal() << "Error in connectivity construction from ItemMultiArrayProperty";
}
...
_node2DoFConnectivityRegistered()
{
  ...
  if (!connectivity_mng->isUpToDate(&node2dof)) {
    ...
    ENUMERATE_NODE (inode, node_family->view(source_family_added_items_lids))
      if (inode->isOwn())
        source_family_added_items_own.add(inode.localId());
    ...
    ENUMERATE_NODE (inode, source_family_added_items_own) {
      uids[i++] = mesh::DoFUids::uid(inode->uniqueId().asInt64());
    }
  ...
  if (!connectivity_mng->isUpToDate(&node2dof))
    fatal() << "Error in connectivity update tracking.";
...
_node2DoFsConnectivityRegistered()
{
  ...
  if (!connectivity_mng->isUpToDate(&node2dofs)) {
    ...
    for (Integer i = 0; i < source_family_removed_items_lids.size(); ++i) {
      for (Integer j = 0; j < nb_dof_per_node; ++j)
        source_family_removed_items_lids_in_connectivity[i * nb_dof_per_node + j] = source_family_removed_items_lids[i];
    }
...
_addNodes(Int32Array2View new_nodes_lids, Int64Array2View new_nodes_uids)
{
  ...
  for (Integer rank = 0; rank < nb_subdomain; ++rank) {
    for (Integer i = 0; i < nb_new_nodes; ++i)
      new_nodes_uids[rank][i] = max_uid * (rank + 1) + i + 1;
                                     ...
                                     Int64ConstArray2View remaining_nodes_uids, Integer item_property_size, bool is_scalar_connectivity)
{
  ...
  if (item_property_size != node2dof.sourceFamily()->nbItem())
    fatal() << "Error : connectivity is not correctly impacted by change in source family";
}
...
_checkTargetFamilyInfo(ItemVectorView tracked_new_dofs, Int32ConstArrayView new_dofs_lids)
{
  ...
  ENUMERATE_DOF (idof, tracked_new_dofs) {
    if (idof->isOwn())
      tracked_new_dofs_lids.add(idof.localId());
  }
  bool is_ok = tracked_new_dofs_lids.size() == new_dofs_lids.size();
  if (is_ok) {
    for (Integer i = 0; i < tracked_new_dofs_lids.size(); ++i)
      is_ok = ((tracked_new_dofs_lids[i] == new_dofs_lids[i]) && is_ok);
  }
  if (!is_ok)
    fatal() << "Error in target family modification tracking";
}
...
doFVariable()
{
  ...
  ENUMERATE_DOF (idof, dof_family->allItems()) {
    dof_array_variable[idof].fill(-1);
  }
  ...
  if (is_scalar_connectivity) {
    ...
    ENUMERATE_NODE (inode, allNodes()) {
      ...
      info() << "== Connectivity value for node (uid) " << inode->uniqueId().asInt64() << " (lid=  " << inode.localId() << ")"
             << " = " << connected_dof_lid;
      if (connected_dof_lid != inode.localId())
        has_error = true;
    }
  }
  else {
