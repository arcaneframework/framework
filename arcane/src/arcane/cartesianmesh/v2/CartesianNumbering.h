...
/*!
 * \brief
 * Handling of a Cartesian numbering on a grid of items
 * of up to 3 dimensions
 * Allows conversion from a triplet (i,j,k) to an ID number
 * The IDs range from firstId() to firstId()+nbItem()
 *
 * Example for a 5x3 numbering in 2D with firstId() = 0:
 *
 *       ids:
 *  2        10 11 12 13 14
 *  1        5  6  7  8  9
...
class CartesianNumbering
{
 ...
 protected:
  ...
  IdType m_first_item_id = 0; //! item_id = m_first_item_id + CartesianNumbering(i,j,k), allows an offset in the numbering
