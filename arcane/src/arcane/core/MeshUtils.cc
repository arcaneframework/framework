...
class _CompareItemWithNodes
{
 ...
 public:
  ...
  : m_items(mesh->itemsInternal(ik)), m_nodes_sorted_id(nodes_sorted_id)
    {
    }
  ItemInternalList m_items;
...
void
_writeItems(std::ostream& ofile,const String& name,Int32ConstArrayView ids)
{
...
class MeshUtilsCheckConnectivity
{
 ...
 public:
  ...
  MeshUtilsCheckConnectivity(IMesh* mesh,const XmlNode& doc_node,bool check_sub_domain)
    : m_mesh(mesh), m_doc_node(doc_node), m_has_error(false)
