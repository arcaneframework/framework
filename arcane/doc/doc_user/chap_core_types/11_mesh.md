...
`dimension()`, which can be 1, 2, or 3. The dimension corresponds to the
dimension of the cell elements.
...
- *secondary meshes* (also called *sub-meshes*), which depend on a primary
  ...
  `defineParentForBuild()`, and the parent/child relationship can be retrieved
  with `parentMesh()` / `parentGroup()` and `childMeshes()`.

`isPrimaryMesh()` indicates if the instance is a primary mesh, and
`toPrimaryMesh()` returns the instance in the form of an \arcane{IPrimaryMesh}
(throwing an exception if it is not one).

## Getting a Mesh {#arcanedoc_core_types_mesh_get}

The default mesh of a subdomain is retrieved with
`subDomain()->defaultMesh()`. It is generally preferable to use
`subDomain()->defaultMeshHandle()`, which returns a \arcane{MeshHandle}: a
stable reference to the mesh that always exists, even if the associated mesh
has not yet been created.

A subdomain can have several meshes. They are managed by the mesh manager
`subDomain()->meshMng()`, and the list of meshes is available via `meshes()`.
...
- `nodeFamily()`, `edgeFamily()`, `faceFamily()`, `cellFamily()`
...
The number of entities of each base kind is given by `nbNode()`, `nbEdge()`,
`nbFace()`, and `nbCell()` (or `nbItem(kind)` for a generic kind).
...
| Group | Description |
|---|---|
| `allNodes()` / `allEdges()` / `allFaces()` / `allCells()` | All entities of the subdomain (including ghosts) |
| `ownNodes()` / `ownEdges()` / `ownFaces()` / `ownCells()` | Entities owned by the subdomain |
| `outerFaces()` | Faces on the boundary of the subdomain |

Other groups can be created and searched by name:

- `groups()`: the collection of all groups,
- `findGroup(name)`: the group named \a name, or a null group if none exists,
- `destroyGroups()`: destroys all groups.
...
(`isDynamic()`). Modifications are performed through the \arcane{IMeshModifier}
interface, retrieved with `modifier()`:
...
Once the modifications are made, `endUpdate()` must be called. This method is
collective and, depending on the mesh properties (see below), sorts and/or
compacts the entities, updates the groups, resizes the variables, and updates
the ghost layers.

\warning Sorting or compacting the entities modifies the `localId()` of all
geometric entities of the mesh. Therefore, you must not retain an entity via
the \arcane{Item} class (or a derived class) or its `localId()` after calling
this method. Use groups (or unique numbers) instead.
...
with `setDimension()` before allocating cells.
...
A mesh has properties, accessible via `properties()`:
...
- \b "compact" (boolean, true by default): entities must be compacted after a
  ...
  (the `localId()` are renumbered from 0 to the number of entities). Compacting
  is a costly operation because it requires updating all variables.
...
`itemsNewOwner(kind)` variable of the primary mesh contains, for each entity,
the number of its new owning subdomain. These variables must be synchronized,
and the redistribution is then performed with `exchangeItems()`:
...
- the method triggers an implicit call to `endUpdate()`.

After the call, you may need to execute the mesh change entry points
(\arcane{ITimeLoopMng::execMeshChangedEntryPoints}).

## Other Features {#arcanedoc_core_types_mesh_other}

- **AMR**: if adaptive mesh refinement is activated (`isAmrActivated()`), the
  mesh provides the groups of active cells (`allActiveCells()`,
  `ownActiveCells()`) and of cells at a given level (`allLevelCells(level)`).
  See \ref arcanedoc_entities_amr_cartesianmesh.
- **Ghost layers**: `ghostLayerMng()` gives access to the ghost layer manager
  and `updateGhostLayers()` rebuilds the ghost layers.
- **Connectivity**: `connectivity()` returns the connectivity descriptor
  (a \arcane{VariableScalarInteger} using the %Arcane connectivity numbering,
  see \ref arcanedoc_entities_connectivity_internal), and
  `indexedConnectivityMng()` the indexed incremental connectivity manager.
- **Unique IDs**: `meshUniqueIdMng()` manages the unique numbering of the
  entities.
- **Semi-conforming interfaces**: `computeTiedInterfaces()`,
  `hasTiedInterface()`, and `tiedInterfaces()` manage the tied interfaces
  between non-conforming sub-meshes.
- **Partitioning constraints**: `partitionConstraintMng()` manages the
  constraints on the mesh partitioning.
- **Utilities and checks**: `utilities()` gives access to mesh utility
  functions (\arcane{IMeshUtilities}), `checker()` to the mesh validity
  checker, `checkValidMesh()` (local) and `checkValidMeshFull()` (collective,
  across all subdomains) to validate the internal structures.
- **Events**: `eventObservable(type)` provides observables for mesh events.
- **User data**: `userDataList()` allows associating user data with the mesh.
- **Family network**: `itemFamilyNetwork()` gives access to the graph of the
  connected families.
...
- Use `subDomain()->defaultMeshHandle()` (a \arcane{MeshHandle}) rather than
  `defaultMesh()` when the mesh may not have been created yet.
- Primary meshes are created by the data set or via
  \arcane{IMainFactory::createMesh}(); sub-meshes are subsets of a primary
  mesh.
- Modify the mesh only through `modifier()`, and always call `endUpdate()`
  after a modification.
- Do not retain \arcane{Item} instances or `localId()` values across an
  `endUpdate()`: use groups or unique numbers.
