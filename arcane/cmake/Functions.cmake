...
function(arcane_add_arccon_packages target visibility)
  ...
  foreach(package ${ARGN})
    message(STATUS "Check add package '${package}' cache_name=${ARCCON_PACKAGE_${package}_TARGETS}")
    set(_USE_ARCCON_TARGET FALSE)
    if (DEFINED ARCCON_PACKAGE_${package}_TARGETS)
      message(STATUS "Found cache name '${ARCCON_PACKAGE_${package}_TARGETS}'")
      set(_PKG ${ARCCON_PACKAGE_${package}_TARGETS})
      set(_USE_ARCCON_TARGET TRUE)
    else()
      set(_PKG arccon::${package})
    endif()
    message(STATUS "Check add package '${_PKG}' use ARCCON_TARGET?=${_USE_ARCCON_TARGET}")
    if (_USE_ARCCON_TARGET)
      # Si la variable ${ARCCON_TARGET_${package}} est non nulle, alors elle fait toujours
      # référence à une cible existante. On peut donc l'ajouter directement
      foreach(subtarget ${_PKG})
        message(STATUS "-- Add target = '${subtarget}'")
        target_link_libraries(${target} ${visibility} ${subtarget})
