...
_computeCQs(Real3 node_coord[8],Real3 face_coord[6],Cell cell)
{
  ...
  // Calcul des r�sultantes aux sommets :
...
{
  // Copie locale des coordonn�es des sommets d'une maille
  Real3 coord[8];
  // Coordonn�es des centres des faces
  Real3 face_coord[6];

  ENUMERATE_CELL(icell,m_mesh.m_cells_vector){
    Cell cell = *icell;

    // Recopie les coordonn�es locales (pour le cache)
    for( NodeEnumerator i_node(cell.nodes()); i_node.index()<8; ++i_node )
      coord[i_node.index()] = m_node_coord[i_node];

    // Calcul les coordonn�es des centres des faces
    ...
    // Calcule les r�sultantes aux sommets
