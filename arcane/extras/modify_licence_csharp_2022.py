...
def convert_file(filename):
    ...
    if full_content_lines[1].find(new_licence_date_string) != (-1):
        print(str.format("File '{0}' already has licence text",filename))
        return False

    if full_content_lines[1].find(licence_detect_string) != (-1):
        print(str.format("File '{0}' already has old licence text",filename))
        full_content_lines[1] = new_licence_date_string + "\n"
    else:
        return False
