...
addItemGroupProperty(ItemGroup group, Integer property)
{
  ...
  while (property) {
    if (property & current_property) {
      ...
      traceMng()->debug()
      << "Add geometric property [" << IGeometryProperty::name(static_cast<IGeometryProperty::eProperty>(current_property))
      << "] on group " << group.name();
    }
                                             ...
                                             IGeometryProperty::eStorage storage)
{
  ...
  if ((properties.defined & property) != property)
    traceMng()->fatal() 
      << "Non available property [" << IGeometryProperty::name(property) 
      << "] requested from group " << group->name();
  ItemGroupGeometryProperty::StorageInfo& storages = properties.storages[property];
  if ((storages.storageType & storage) != storage)
    traceMng()->fatal() 
      << "Non available storage [" << IGeometryProperty::name(storage) 
      << "] requested for property " << IGeometryProperty::name(property) 
      << " on group " << group->name();
  return storages;
}
  ...
  for (PropertyMap::iterator igroup = m_group_property_map.begin(); igroup != m_group_property_map.end(); ++igroup) {
    ...
    for (ItemGroupGeometryProperty::StorageInfos::const_iterator i = group_property.storages.begin();
      ...
      if (info.usageMapCount == 0 and (info.realMap or info.real3Map) and not(info.externStorage & IGeometryProperty::PVariable))
        traceMng()->warning() 
          << "Geometric property " << IGeometryProperty::name(property) 
          << " on group " << igroup->first->name() << " with Map storage never used since last update";
      ...
      if (info.realMap or info.real3Map and (info.externStorage & IGeometryProperty::PItemGroupMap)) {
        ...
        if (finder != extern_maps.end()) {
          traceMng()->warning() 
            << "Geometric property " << IGeometryProperty::name(property) 
            << " on group " << igroup->first->name() 
            << " shares extern ItemGroupMap " << finder->first->name() 
            << " with property " << IGeometryProperty::name(finder->second.second) 
            << " on group " << finder->second.first->name();
        }
        else {
