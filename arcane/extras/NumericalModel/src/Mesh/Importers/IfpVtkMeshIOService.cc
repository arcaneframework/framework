...
private:
  ...
  bool _readStructured3D(IPrimaryMesh* mesh,IfpVtkFile& vtk_file,bool use_internal_partition,
                         ...
                         CArrayInteger& cells_local_id, CArrayInteger& nodes_local_id) ;
  ...
  bool _readUnstructuredGrid(IPrimaryMesh* mesh,IfpVtkFile& vtk_file,bool use_internal_partition);
  ...
  void _readNodesUnstructuredGrid(IPrimaryMesh* mesh,IfpVtkFile& vtk_file,CArrayReal3& node_coords);
            ...
            const String& expected_value4,const String& expected_value5, const String& expected_value6)
{
  ...
  if (current_value_low.toString()!=expected_value1_low.toString()
    ...
    String s = "Attend chaine '" + expected_value1 + "' ou '"
      + expected_value2 + "' ou '" + expected_value3 + "' ou '"
      + expected_value4 + "' ou '" + expected_value5 + "' ou '"
      + expected_value6  
      + "', trouve '" + current_value + "'";
    throw IOException("IfpVtkFile::checkString()",s);
  }
}
...
_readMesh(IPrimaryMesh* mesh,const String& file_name,const String& dir_name,bool use_internal_partition)
{
  ...
  if (!ifile){
    error() << "Unable to open in read mode file '" << file_name << "'";
    return true;
  }
...
_readStructuredGrid(IPrimaryMesh* mesh,IfpVtkFile& vtk_file,bool use_internal_partition)
{
  ...
  CArrayInteger nodes_local_id;
  {
    ...
    if (!iline){
      error() << "Syntax error detected while reading mesh dimensions";
      return true;
    }
  ...
  do {
    ...
    if( IfpVtkFile::isEqualString(keyword,"TAGS") ) {
      ...
      if(!iline) {
        error() << "Syntax error detected while reading boundary faces tags";
        return true;
      }
      info() << "Boundary faces tags:";
      ...
      iline >> ws >> nb_groups;
      if (!iline){
        error() << "Syntax error while reading cell groups";
        return true;
      }
      ...
      iline >> ws >> nb_groups;
      if (!iline){
        error() << "Syntax error while reading node groups";
        return true;
      }
      for(Integer igroup=0;igroup<nb_groups;igroup++)
      ...
      iline >> ws >> nb_groups;
      if (!iline){
        error() << "Syntax error while reading face groups";
        return true;
      }
      for(Integer igroup=0;igroup<nb_groups;igroup++)
        _createItemGroup(mesh, vtk_file, buf, nb_cell_x, nb_cell_y, nb_cell_z,
  ...
  info() << "Building groups for boundary conditions";
      ...
      if(IfpVtkFile::isEqualString(origin_str,"TOP"))
        {
          ...
          if (!iline){
            error() << "Syntax error while reading TOP data";
            return true;
          }
        }
    }
            ...
            if(IfpVtkFile::isEqualString(stepz_str,"DZFILE"))
              {
                ...
                if (!dz_file_read) {
                  error() << "Error while reading file DZFILE : " << dz_file_str;
                  return true;
                }
                layer_top = (dzf[0]<0) ;
              }
            ...
            if(IfpVtkFile::isEqualString(stepx_str,"DXFILE"))
              {
                ...
                if (!dx_file_read) {
                  error() << "Error while reading file DXFILE : " << dx_file_str;
                  return true;
                }
                layer_top = (dxf[0]<0) ;
              }
            ...
            if(IfpVtkFile::isEqualString(stepy_str,"DYFILE"))
              {
                ...
                if (!dy_file_read) {
                  error() << "Error while reading file DYFILE : " << dy_file_str;
                  return true;
                }
                layer_top = (dyf[0]<0) ;
              }
        ...
        if(top_def)
          {
            ...
            if (!top_file_read)
              {
                error() << "Error while reading file TOP : " << top_file_str;
                return true;
              }
          }
        if(dx_def)
                  ...
                  CArrayInteger& cells_local_id, CArrayInteger& nodes_local_id)
{
...

  {
    ...
    //             1 pour son type,
    //             4 pour chaque noeud
    CArrayInt64 cells_infos(nb_cell*6);
    Integer off_layer = 0 ;
    if(layer_top)
      off_layer = 2 ;
    {
...
_readStructuredDualGrid(IPrimaryMesh* mesh,IfpVtkFile& vtk_file,bool use_internal_partition)
{
  ...
  if(graph==NULL)
    {
      error() << "Mesh type 'DynamicsGraph' required ";
      return true ;
    }
  // IParallelMng* pm = subDomain()->parallelMng();
...
_readNodesUnstructuredGrid(IPrimaryMesh* mesh,IfpVtkFile& vtk_file,CArrayReal3& node_coords)
{
  ...
  if (nb_node<0){
    String msg = "Invalid number of nodes: n=" + nb_node;
    throw IOException(func_name,msg);
  }

  info() << " number of nodes: " << nb_node;
                           ...
                           CArrayInt64& cells_connectivity)
{
  ...
  // Lecture du type des mailles
  {
    ...
    if (nb_cell_type!=nb_cell){
      StringBuilder msg = "Inconsistent number CELL_TYPES: v=";
      msg += nb_cell_type;
      msg += " nb_cell=";
      msg += nb_cell;
      throw IOException(func_name,msg.toString());
  ...
  for( Integer i=0; i<nb_cell; ++i ){
    ...
    default:
      {
        String msg = "Cell type vtk invalid or unknown: type=" + vtk_ct;
        throw IOException(func_name,msg);
      }
      break;
    }
...
bool IfpVtkMeshIOService::
_readUnstructuredGrid(IPrimaryMesh* mesh,IfpVtkFile& vtk_file,bool use_internal_partition)
{
...
_readStructuredFacesMesh(IPrimaryMesh* mesh,const String& file_name,const String& dir_name,bool use_internal_partition)
{
  ...
  if(sid==0)
  {
    ...
    info() << " Info: " << nb_node_x << " " << nb_node_y << " " << nb_node_z;
  ...
  if (sid==0){
    ...
    while(continue_loop) {
      ...
      if (IfpVtkFile::isEqualString(data_str,"POINT_DATA")){
        ...
        error() << "Expected value POINT_DATA or CELL_DATA, found: '" << data_str << "'";
        return true;
      }
                                         ...
                                         OCStringStream& created_infos_str)
{
  ...
  info() << "** ** ** READ NAME: name=" << name_str << " type=" << type_str;
  ...
  if (is_group)
    {
      if (not IfpVtkFile::isEqualString(type_str,"int"))
        {
          error() << "Expecting group type 'int', found: " << type_str;
          return true;
        }
          ...
          if (is_group){
            ...
            if (reading_cell){
              ...
              if (item_kind!=IK_Cell)
                throw IOException("No support to read face variables");
                                                   ...
                                                   CArrayInteger& links_type)
{
  ...
  info() << "Reading values for variable: " << name_str << " n=" << nb_cell_kind;
  ...
  info() << "Build finished for variable: " ;
...
_readNodeVariable(IPrimaryMesh* mesh,IfpVtkFile& vtk_file,const String& var_name,Integer nb_node)
{
  ...
  info() << "Reading values for variable: " << var_name << " n=" << nb_node;
  ...
  info() << "Build finished for variable: " << vtk_file.isEnd();
}
...
{
  //TODO Faire la conversion uniqueId() vers localId() correcte
  info() << "Reading the value of the variable: " << var_name << " n=" << nb_cell;
  ...
  info() << "Build finished for variable: " << var_cell->variable()->fullName() << " " << vtk_file.isEnd();
}
                 ...
                 bool use_internal_partition)
{
  ...
  info() << "Building group: " << group_name << " nb_element=" << ids.size();
...
_readDistStructuredGrid(IPrimaryMesh* mesh,IfpVtkFile& vtk_file,bool use_internal_partition)
{
...

  {
    ...
    // Positionne les coordonn?es
    info()<<"Start building mesh coordinates";
    {
