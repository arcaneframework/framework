...
// -*- C++ -*-
#ifndef ARCGEOSIM_UTILS_ITEMGROUPMAP_H
#define ARCGEOSIM_UTILS_ITEMGROUPMAP_H
...
#else /* ARCANE_VERSION */
  virtual void _executeReduce(const Int32ConstArrayView * old_to_new_ids, const Int32ConstArrayView * info2) = 0;
...
#else /* ARCANE_VERSION */
    virtual void _executeReduce(const Int32ConstArrayView * removed_ids_ptr, const Int32ConstArrayView * info2)
#endif /* ARCANE_VERSION */
  {
...
protected:
  virtual void _executeExtend(const Int32ConstArrayView * new_ids_ptr)
  {
    BaseClass::_executeExtend(new_ids_ptr);
    _dataShifter(+new_ids_ptr->size());
  }

#if (ARCANE_VERSION<10800)
  virtual void _executeReduce(const Int32ConstArrayView * removed_ids_ptr)
  {
    BaseClass::_executeReduce(removed_ids_ptr);
    _dataShifter(-removed_ids_ptr->size());
  }
#else /* ARCANE_VERSION */
  virtual void _executeReduce(const Int32ConstArrayView * removed_ids_ptr, const Int32ConstArrayView * info2)
  {
    BaseClass::_executeReduce(removed_ids_ptr, info2);
    _dataShifter(-removed_ids_ptr->size());
  }
#endif /* ARCANE_VERSION */
...
private:
  void _dataShifter(const Integer incr)
  {
    ItemGroup group(this->group());
    Array<ValueType> old_array = m_array_data; // sans copie grace à Array
    const Integer old_group_size = group.size()-incr;
    const Integer global_size = group.size() * m_array_size;
    ARCANE_ASSERT((old_group_size*m_array_size==old_array.size()),("Incompatible group and increment"));
    m_array_data = Array<ValueType>(global_size);
    ValueType * base_data = m_array_data.unguardedBasePointer();

    // On doit utiliser l'accès par ItemEnumerator, car c'est des accès directs
    ItemEnumerator i(group.enumerator());
    const Integer old_copy_max_index = math::min(old_group_size,group.size());   
    for( ; i.index()<old_copy_max_index; ++i) 
      {
        // Copy from old structure
        ARCANE_ASSERT((i.hasNext()),("Internal inconsistency"));
        ValueType * currentData = base_data + m_array_size * i.index();
...
#endif /* ARCGEOSIM_UTILS_ITEMGROUPMAP_H */
