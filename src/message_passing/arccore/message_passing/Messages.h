...
#define ARCCORE_GENERATE_MESSAGEPASSING_PROTOTYPE(type) \
  inline void mpAllGather(IMessagePassingMng* pm,ConstSpan<type> send_buf,Span<type> recv_buf) \
  { type* x = nullptr; pm->dispatchers()->dispatcher(x)->allGather(send_buf,recv_buf); } \
  inline void mpGather(IMessagePassingMng* pm,ConstSpan<type> send_buf,Span<type> recv_buf,Int32 rank) \
  { type* x = nullptr; pm->dispatchers()->dispatcher(x)->gather(send_buf,recv_buf,rank); } \
  inline void mpAllGatherVariable(IMessagePassingMng* pm,ConstSpan<type> send_buf,Array<type>& recv_buf) \
  { type* x = nullptr; pm->dispatchers()->dispatcher(x)->allGatherVariable(send_buf,recv_buf); } \
  inline void mpGatherVariable(IMessagePassingMng* pm,ConstSpan<type> send_buf,Array<type>& recv_buf,Int32 rank)\
  { type* x = nullptr; pm->dispatchers()->dispatcher(x)->gatherVariable(send_buf,recv_buf,rank); } \
  inline void mpScatterVariable(IMessagePassingMng* pm,ConstSpan<type> send_buf,Span<type> recv_buf,Int32 root); \
  ...
  inline void mpSend(IMessagePassingMng* pm,ConstSpan<type> values,Int32 rank) \
  ...
  inline Request mpSend(IMessagePassingMng* pm,ConstSpan<type> values,Int32 rank,bool is_blocked) \
  ...
  inline void mpAllToAll(IMessagePassingMng* pm,ConstSpan<type> send_buf,Span<type> recv_buf,Int32 count) \
  { type* x = nullptr; return pm->dispatchers()->dispatcher(x)->allToAll(send_buf,recv_buf,count); } \
  inline void mpAllToAllVariable(IMessagePassingMng* pm,ConstSpan<type> send_buf,ConstArrayView<Int32> send_count, \
