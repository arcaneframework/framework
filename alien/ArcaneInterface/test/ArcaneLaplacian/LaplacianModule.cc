...
void
LaplacianModule::hydroStartInit()
{
...
void
LaplacianModule::hydroContinueInit()
{
  ENUMERATE_CELL (icell, allCells()) {
    const Cell& cell = *icell;
    ...
    for (Integer inode = 0; inode < 8; ++inode)
      volume += math::scaMul(m_node_coord[cell.node(inode)], m_cell_cqs[icell][inode]);
    volume /= 3.;
...
void
LaplacianModule::computePressureForce()
{
...
void
LaplacianModule::computeVelocity()
{
...
void
LaplacianModule::applyBoundaryCondition()
{
  for (Integer i = 0, nb = options()->boundaryCondition.size(); i < nb; ++i) {
    ...
    TypesLaplacian::eBoundaryCondition type = options()->boundaryCondition[i]->type();
    ...
    ENUMERATE_FACE (j, face_group) {
      ...
      for (Integer k = 0; k < nb_node; ++k) {
        ...
        switch (type) {
        case TypesLaplacian::VelocityX:
          velocity.x = value;
          break;
        case TypesLaplacian::VelocityY:
          velocity.y = value;
          break;
        case TypesLaplacian::VelocityZ:
          velocity.z = value;
          break;
        case TypesLaplacian::Unknown:
          break;
        }
      }
    }
...
void
LaplacianModule::computeGeometricValues()
{
...
void
LaplacianModule::applyEquationOfState()
{
...
inline void
LaplacianModule::computeCQs(Real3 node_coord[8], Real3 face_coord[6], const Cell& cell)
{
...
ARCANE_REGISTER_MODULE_LAPLACIAN(LaplacianModule);
