...
#include "TypesLaplacian.h"
#include "Laplacian_axl.h"
 ...
 *   - le seul type de calcul de longueur caractéristique supporté est celui utilisant les
 * médianes,
 *   - le seul type de condition aux limites supporté est d'imposer une composante de la
 * vitesse sur une surface,
 *   - la masse nodale est supposée constante et n'est pas recalculée à chaque itération,
 *   - aucun test de cohérence des valeurs (pression positive, volume positif, ...)  n'est
 * effectué.
 *
 ...
 *   - calcul des nouvelles valeurs géométriques : volume des mailles, longueur
 * caractéristique des mailles,
 *     resultantes aux sommets de chaque maille,
 *   - calcul de la densité,
 *   - calcul de la pression et de l'énergie par l'équation d'état. Ce calcul est effectué
 * par un service
 *     ARCANE. Deux implémentations sont disponibles pour le service : gaz parfait, et
 * "stiffened" gaz.
 *   - calcul du nouveau pas de temps.
 *
 */
class LaplacianModule : public ArcaneLaplacianObject
{
 public:
  /** Constructeur de la classe */
  LaplacianModule(const ModuleBuildInfo& mbi)
  : ArcaneLaplacianObject(mbi)
  {
  }
