...
std::ostream&
operator<<(std::ostream& o, const std::array<T, N>& a)
{
  for (auto x : a)
    o << " " << x;
  return o << " ";
...
template <typename T>
std::ostream&
operator<<(std::ostream& o, const std::vector<T>& a)
{
  for (auto x : a)
    o << " " << x;
  return o << " ";
...
struct OptionTools
{
  ...
  {
    static std::array<typename T::type, N> requiredFixedArray(
  ...
  /*    ////// requiredVariableArray
     ...
     std::vector<typename T::type> requiredVariableArray(Args&&... args) { return
     Internal2<T,Nmin,Nmax,0,Args...>::requiredVariableArray(std::vector<typename
     T::type>{}, std::move(args)...); }
      ...
      struct Internal2<T,Nmin,Nmax,CurN,Head,Tail...> { static std::vector<typename
     T::type> && requiredVariableArray(std::vector<typename T::type> && r, Head && t, Tail
     &&... args) { return
     ...
     std::move(args)...); } };
      ...
      struct Internal2<T,Nmin,Nmax,CurN,T,Tail...> { static std::vector<typename T::type>
     && requiredVariableArray(std::vector<typename T::type> && r, T && t, Tail &&... args)
     { r.emplace_back(t.value); return
     Internal2<T,Nmin,Nmax,CurN+1,Tail...>::requiredVariableArray(std::move(r),std::move(args)...);
      ...
      struct Internal2<T,Nmin,Nmax,CurN> { static std::vector<typename T::type> &&
     requiredVariableArray(std::vector<typename T::type> && r) { static_assert(CurN>=Nmin
     && CurN<=Nmax, "Invalid required parameter count"); return std::move(r); } };
  */
  ////// requiredVariableArray
  template <typename T, int Nmin, int Nmax, typename... Args>
  static Arccore::UniqueArray<typename T::type> requiredVariableArray(Args&&... args)
  {
    return Internal2<T, Nmin, Nmax, 0, Args...>::requiredVariableArray(
        Arccore::UniqueArray<typename T::type>{}, std::move(args)...);
  }

  template <typename T, int Nmin, int Nmax, int CurN, typename... Args> struct Internal2
  {
  };
  ...
  {
    static Arccore::UniqueArray<typename T::type>&& requiredVariableArray(
        Arccore::UniqueArray<typename T::type>&& r, Head&& t, Tail&&... args)
    {
  ...
  {
    static Arccore::UniqueArray<typename T::type>&& requiredVariableArray(
        Arccore::UniqueArray<typename T::type>&& r, T&& t, Tail&&... args)
    {
      r.add(t.value);
  ...
  {
    static Arccore::UniqueArray<typename T::type>&& requiredVariableArray(
        Arccore::UniqueArray<typename T::type>&& r)
    {
      static_assert(CurN >= Nmin && (Nmax == -1 || CurN <= Nmax),
  ...
  template <typename T, int CurN, typename... Tail> struct Internal3<T, CurN, T, Tail...>
  {
  ...
  template <typename... AllowedTypes> struct Internal4<std::tuple<AllowedTypes...>>
  {
...
template <typename Name, typename Type> struct OptionValue
{
  ...
  // std::move(*this); }
  // friend std::ostream & operator<<(std::ostream & o, const OptionValue<Name, Type> & x)
  // { return o << x.value; }
};
...
template <typename Name, typename Type> struct OptionProxy
{
  ...
  // OptionValue<Name, Type> operator=(Type && value) {  // possible to optimize in one
  // line as above
 ...
 public:
  static std::unique_ptr<Interface> create(const std::string& name)
  {
    ...
    /*if (finder == ctor_functors.end())
      throw InstanceParameterException{stringer("Cannot find implementation '", name, "'
      for interface ", typeid(Interface).name())};*/
...
struct InstanceRegisterer
{
  InstanceRegisterer(const std::string& name)
  {
    InstanceBuilder<Interface>::registerImplementation(name, []() -> Interface* {
      return new Class(
          std::move(std::unique_ptr<SrongOptionClass>(new SrongOptionClass{})));
    });
  }
};
