...
int main(int argc, char** argv)
{
  return Environment::execute(argc, argv, [&] {
      ...
      default :
      {
        ...
        // FILL MATRIX
        Alien::ProfiledMatrixBuilder matrix(std::move(matrixA), Alien::ProfiledMatrixOptions::eResetValues);
        for (int irow = offset; irow < offset + local_size; ++irow)
        {
      ...
      double normx_ref = 0;
      if(!(builder_type==0) || comm_size==1 )  // le direct builder ne permet pas de faire des produits matrix vector en parallel
      {
