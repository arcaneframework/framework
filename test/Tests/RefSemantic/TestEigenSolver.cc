...
TEST(TestEigenSolver, HTSEigenSolverSmallestValue) {
  ...
  std::shared_ptr<IOptionsHTSEigenSolver>
      options(new StrongOptionsHTSEigenSolver
                                             {
                                                _output          = 1,
...
TEST(TestEigenSolver, HTSGenEigenSolverSmallestValue) {
  ...
  //
  solver->init();
  GeneralizedEigenProblemT<Alien::BackEnd::tag::simplecsr,std::vector<double>> problem(A,B) ;
  solver->solve(problem);
  const Alien::IEigenSolver::Status& status = solver->getStatus() ;
  if(status.m_succeeded)
  {
...
TEST(TestEigenSolver, SLEPcEigenSolverSmallestValue) {
  ...
  std::shared_ptr<IOptionsSLEPcEigenSolver>
      options(new StrongOptionsSLEPcEigenSolver
                                             {
                                                _output          = 1,
                                                _maxIterationNum = 1000,
                                                _tol             = 1e-6,
