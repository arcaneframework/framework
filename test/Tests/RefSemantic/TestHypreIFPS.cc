...
TEST(TestHypreIFPS, HypreTest) {
...
#ifdef ALIEN_USE_IFPSOLVER
  {
    ...
    std::shared_ptr<IOptionsIFPLinearSolver> options(new StrongOptionsIFPLinearSolver{
            _output = 1
            , _numIterationsMax = 1000
            , _stopCriteriaValue = 1e-6
            , _precondOption = IFPSolverProperty::AMG
    });
    auto* solver = new Alien::IFPInternalLinearSolver(Environment::parallelMng(), options.get());
    //
    solver->init();
    //solver->solve(A,x,y);
...
#ifdef ALIEN_USE_HYPRE
  {
    ...
    using namespace HypreSolverOptionsNames;
      ...
      auto* solver = new Alien::HypreInternalLinearSolver(Environment::parallelMng(), nullptr);
      //
      solver->init();
      //solver->solve(A,y,x);
  }
