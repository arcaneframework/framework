...
/*---------------------------------------------------------------------------*/
/*---------------------------------------------------------------------------*/
Integer AlienStokesModule::_computeDir(Real3 const& x)
{
  Integer dir = -1 ;
  Real max_coord = 0. ;
...
Integer AlienStokesModule::_computeOrientation(Face const& face)
{
...
/*---------------------------------------------------------------------------*/
bool AlienStokesModule::_isNodeOfFace(Face const& face, Integer node_lid)
{
...
void AlienStokesModule::_computeFaceConnectivity(Face const& face, Alien::UniqueArray2<Integer>& connectivity)
{
  ...
  for(Integer i=0;i<nb_nodes;++i)
  {
    ...
    ENUMERATE_FACE(iface2,node0.faces())
    {
      ...
      if(conn_face.localId()!= face_lid && _isNodeOfFace(conn_face,node1_lid) && m_face_type[conn_face]==face_type)
      {
...
Integer AlienStokesModule::_upStreamFace(Face const& face, Integer cell_id)
{
  Integer up = 0 ;
...
AlienStokesModule::eBCType AlienStokesModule::getBCType(Real3 const& xF, Integer dir)
{
  switch(dir)
  {
    case 0 :
...
AlienStokesModule::eBCType AlienStokesModule::getBCType(Face const& face)
{
  Integer face_type = m_face_type[face] ;
...
AlienStokesModule::initVelocityAndPressure()
{
  ...
  ENUMERATE_CELL(icell,allCells())
  {
    ...
    m_xE[icell]    = m_xV[icell][0]*m_xV[icell][0] + m_xV[icell][1]*m_xV[icell][1] + m_xV[icell][2]*m_xV[icell][2] ;
  }
}
void
...
test()
{
  ...
  m_Adist  = MatrixDistribution (u_global_size, u_global_size, u_local_size, parallelMng()->messagePassingMng());
  m_Bdist  = MatrixDistribution (u_global_size, p_global_size, u_local_size, parallelMng()->messagePassingMng());
  m_tBdist = MatrixDistribution (p_global_size, u_global_size, p_local_size, parallelMng()->messagePassingMng());
  m_udist  = VectorDistribution(u_global_size, u_local_size, parallelMng()->messagePassingMng()) ;
  m_pdist  = VectorDistribution(p_global_size, p_local_size, parallelMng()->messagePassingMng()) ;
  ...
  Alien::UniqueArray<Integer>  allUIndexA = u_index_manager.getIndexes(u_indexSet);
...

    {
      ...
      info()<<"FILL tB FILLER";
      Alien::ProfiledMatrixBuilder buildertB(tB, Alien::ProfiledMatrixOptions::eResetValues);
      ...
      {
        const Face & face = *iface;
        if(face.isSubDomainBoundary()) continue ;

        auto face_lid = iface->localId() ;
        auto face_type = m_face_type[face] ;

        const Integer iIndex = allUIndex[face.localId()]; assert(iIndex!=-1) ;
        rhs[iIndex] += m_f[face] ;

        const Integer iIndexU = allUIndexA[face.localId()]; assert(iIndexU!=-1) ;
        ...
        if(! front_cell.null())
        {
          ...
          builderB(iIndexU,allPIndexB[front_cell.localId()]) = orientation/m_h[face_type] ;
        ...
        if(! back_cell.null())
        {
          builder(iIndex,allPIndex[back_cell.localId()]) = -1.*orientation/m_h[face_type] ;
          builderB(iIndexU,allPIndexB[back_cell.localId()]) = -1.*orientation/m_h[face_type] ;
      ...
      ENUMERATE_CELL(icell,areaP.own())
      {
        ...
        const Integer iIndex = allPIndex[cell.localId()]; assert(iIndex!=-1) ;

        const Integer iIndexP = allPIndexB[cell.localId()]; assert(iIndexP!=-1) ;
...
Real AlienStokesModule::pressure(Real3 const& x) const
{
  return 1-x[0] ;
}

Real AlienStokesModule::ux(Real3 const& x) const
{
  return 1+x[0] ;
}

Real AlienStokesModule::duxdn(Real3 const& x, Integer dir) const
{
  switch(dir)
  {
    case 0 :
...
Real AlienStokesModule::uy(Real3 const& x) const
{
  return x[1]-0.5 ;
}

Real AlienStokesModule::duydn(Real3 const& x, Integer dir) const
{
  switch(dir)
  {
    case 1 :
...
Real AlienStokesModule::uz(Real3 const& x) const
{
  return x[2]-0.5 ;
}

Real AlienStokesModule::duzdn(Real3 const& x, Integer dir) const
{
  switch(dir)
  {
    case 2 :
...
Real AlienStokesModule::div(Real3 const& xC) const
{
  return 3 ;
}

Real AlienStokesModule::func(Real3 const& xC, Integer dir) const
{
  switch(dir)
  {
    case 0 :
...
Real AlienStokesModule::funcN(Real3 const& xF, Integer dir) const
{
  switch(dir)
  {
    case 0 :
...
Real AlienStokesModule::funcD(Real3 const& xF, Integer dir) const
{
  switch(dir)
  {
    case 0 :
...
bool AlienStokesModule::solveUzawaMethod( Alien::Matrix& A,
                                          Alien::Matrix& B,
                                          Alien::Matrix& tB,
                                          Alien::Vector& f,
                                          Alien::Vector& g,
