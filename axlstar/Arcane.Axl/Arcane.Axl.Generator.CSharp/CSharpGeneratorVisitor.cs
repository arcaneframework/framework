...
namespace Arcane.Axl
{
  ...
  public class CSharpOptionBuilderGenerator : IOptionInfoVisitor
  {
    ...
    public virtual void VisitComplex(ComplexOptionInfo info)
    {
      if (info.Name != null) {
        string var_name = CSharpCodeGenerator.ToFuncName(info.Name);
        string type_name = CSharpVariableTypeGenerator.TypeName(info);
        m_stream.Write("  m_" + var_name + " ");
        m_stream.Write($" = new {type_name}({m_parent_list},\"{info.Name}\", {m_pos});\n");
      }
    }
    ...
    private void _visitOptionInfo(Option info)
    {
      string var_name = CSharpCodeGenerator.ToFuncName(info.Name);
      string type_name = CSharpVariableTypeGenerator.TypeName(info);
      m_stream.Write($" m_{var_name} = new {type_name}");
      m_stream.Write("(new Arcane.CaseOptionBuildInfo(");
  ...
  public class CSharpClassDefinitionGenerator : IOptionInfoVisitor
  {
    ...
    public virtual void VisitComplex(ComplexOptionInfo info)
    {
      ...
      if (info.IsMulti) {
        ...
        m_stream.Write($"    public class {v_class_name} : Arcane.CaseOptionComplexValue \n");
        m_stream.Write("    {\n");
        m_stream.Write($"    public {v_class_name}(Arcane.ICaseMng cm,Arcane.ICaseOptionsMulti multi,Arcane.ICaseOptionList picl,Arcane.XmlNode pelement)\n");
        m_stream.Write("    : base(multi,picl,pelement)\n");
        m_stream.Write("{\n");
        m_stream.Write("  Arcane.XmlNode element = Element();\n");
        m_stream.Write("  Arcane.ICaseOptionList icl = ConfigList();\n");

        CSharpOptionBuilderGenerator bg = new CSharpOptionBuilderGenerator(m_stream, "icl", "element");
        ...
        m_stream.Write($"  public class {c_class_name} : Arcane.CaseOptionsMulti\n");
        m_stream.Write("  {\n");
        m_stream.Write($"  {v_class_name}[] m_values;\n");
        m_stream.Write("\n");
        m_stream.Write("    public " + c_class_name);
        m_stream.Write("(Arcane.ICaseOptionList icl,string s,Arcane.XmlNode element)\n");
        m_stream.Write("    : base(icl,s,element," + info.MinOccurs
                       + "," + info.MaxOccurs + ") {\n");
        CSharpCodeGenerator.writeNameTranslations(m_stream,
                                             info.m_alternative_names,
                                             "");
        ...
        m_stream.Write("    {\n");
        m_stream.Write("      Arcane.ICaseOptionsMulti co_multi = this.ToCaseOptionsMulti();\n");
        ...
        m_stream.Write($"        m_values[i] = new {v_class_name}(CaseMng(),co_multi,ConfigList(),elist.Node(i));\n");
        m_stream.Write("      }\n");
        m_stream.Write("    }\n");
