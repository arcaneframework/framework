  ...
  */
  public abstract class Option : XmlInfo
  {
    ...
    void _Init(XmlElement node)
    {
      ...
      if (m_description_element != null)
      {
         XmlElement option_index = Utils.GetElementIfExists(m_description_element,"option-index"); 
        if (option_index != null) 
        {
          bool is_ok = int.TryParse(option_index.InnerText,out  m_option_index);
          Console.WriteLine("Option index "+m_name+ " index = " +m_option_index);
          if (!is_ok) {
            Console.WriteLine("Valeur invalide pour la balise description/option-index. La valeur doit être un entier");
          }
        }
      }
    ...
    /// Index de l'option, utile pour définir l'ordre des options dans leur parent pour la génération du xsd
    private int m_option_index = 0;
    public int OptionIndex {get {return m_option_index;}}
