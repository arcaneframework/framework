...
namespace Arcane.Axl
{
  ...
  public class ServiceOrModuleInfo : XmlInfo
  {
    ...
    string m_tag_name;
    //! Nom de l'Ã©lÃ©ment XML racine (stocké pour des raisons de compatibilité). Vaut "module" ou "service".
    ...
    /** Liste des interfaces qu'implémente le service ou le module. */
    protected List<Interface> m_interfaces = new List<Interface>();
    ...
    public ServiceOrModuleInfo (XmlElement node, string file_base_name, bool is_module)
    {
      ...
      XmlElement interface_node = Utils.GetElementIfExists (node, "interface");
      if (interface_node != null) {
        m_type = Utils.XmlGetAttributeValue (interface_node, "name");
      } else {
        m_type = m_name + "-type";
      }
    }
    ...
    //! Ajoute l'interface spécifiéee dans l'attribut \a name de l'élément \a e
    protected void _AddInterface(XmlNode e)
    {
