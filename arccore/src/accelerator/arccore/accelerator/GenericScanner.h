             ...
             DataType init_value, Operator op, const TraceInfo& trace_info)
  {
    ...
    case eExecutionPolicy::CUDA: {
      ...
      if constexpr (IsExclusive)
        ...
        ARCCORE_CHECK_CUDA(::cub::DeviceScan::InclusiveScan(nullptr, temp_storage_size,
                                                            input_data, output_data, op, nb_item, stream));
      void* temp_storage = m_storage.allocate(temp_storage_size);
      if constexpr (IsExclusive)
        ARCCORE_CHECK_CUDA(::cub::DeviceScan::ExclusiveScan(temp_storage, temp_storage_size,
                                                            input_data, output_data, op, init_value, nb_item, stream));
      else
        ARCCORE_CHECK_CUDA(::cub::DeviceScan::InclusiveScan(temp_storage, temp_storage_size,
                                                            input_data, output_data, op, nb_item, stream));
    } break;
#endif
    ...
    case eExecutionPolicy::HIP: {
      ...
      if constexpr (IsExclusive)
        ...
        ARCCORE_CHECK_HIP(rocprim::inclusive_scan(nullptr, temp_storage_size, input_data, output_data,
                                                  nb_item, op, stream));
      void* temp_storage = m_storage.allocate(temp_storage_size);
      if constexpr (IsExclusive)
        ARCCORE_CHECK_HIP(rocprim::exclusive_scan(temp_storage, temp_storage_size, input_data, output_data,
                                                  init_value, nb_item, op, stream));
      else
        ARCCORE_CHECK_HIP(rocprim::inclusive_scan(temp_storage, temp_storage_size, input_data, output_data,
                                                  nb_item, op, stream));
    } break;
#endif
#if defined(ARCCORE_COMPILING_SYCL)
    case eExecutionPolicy::SYCL: {
#if defined(ARCCORE_USE_SCAN_ONEDPL) && defined(__INTEL_LLVM_COMPILER)
