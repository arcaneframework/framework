...
template<typename ValueT,typename TagT>
ValueT TrilinosVector<ValueT,TagT>::norm1() const {
  return m_internal->m_internal->norm1() ;
}

template<typename ValueT,typename TagT>
ValueT TrilinosVector<ValueT,TagT>::norm2() const {
  return m_internal->m_internal->norm2() ;
}

template<typename ValueT,typename TagT>
ValueT TrilinosVector<ValueT,TagT>::dot(TrilinosVector const& y) const {
...
{
  Tpetra::MatrixMarket::Writer<typename VectorInternal::matrix_type>::writeDenseFile(filename,*m_internal->m_internal);
}
