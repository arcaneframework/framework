...
test() {
    ...
    auto indexSetX = index_manager.buildScalarIndexSet("X",
                                                       Alien::ArcaneTools::SimpleAbstractFamily(xUids, &index_manager));
    ...
    Alien::MatrixData matrixA(space,
                              m_mdist); // local matrix for exact measure without side effect (however, you can reuse a matrix with several builder)
    if (m_vect_size == 0) {
    ...
    for (Integer ibuilder = 0; ibuilder < options()->builder.size(); ++ibuilder) {
        ...
        //
        {
            ...
            if (builder_type == AlienTestOptionTypes::StreamBuilder)
                continue; // XT (28/06/2016) : no more test in that framework for StreamBuilder (as it won't work the same way than other builders)
            switch (builder_type) {
                ...
                case AlienTestOptionTypes::ProfiledBuilder: {
                    ...
                    for (Integer loop = 0; loop < options()->repeatLoop(); ++loop) {
                        ...
                        if (m_vect_size == 0) {
                            ...
                            Alien::ProfiledMatrixBuilder builder(std::move(matrixA),
        ...
        if (subDomain()->parallelMng()->commRank() == 0) {
            info() << "========= Alien Statistics Summary with " << builderNodeList[ibuilder].value()
                   << " =============";
            printf("\tIndexManager time=%5.2f mem=%7.2f max_mem=%7.2f\n", index_manager_stat.time,
                   index_manager_stat.memory, index_manager_stat.memory_max);
            printf("\tProfiler     time=%5.2f mem=%7.2f max_mem=%7.2f count=%d\n", profile_stat.time,
                   profile_stat.memory, profile_stat.memory_max, profile_stat.allocation_count);
            for (Integer i = 0; i < inserter_stats.size(); ++i)
                printf("\tFilling %2d   time=%5.2f mem=%7.2f max_mem=%7.2f count=%d\n", i, inserter_stats[i].time,
                       inserter_stats[i].memory, inserter_stats[i].memory_max, inserter_stats[i].allocation_count);
...
void
AlienTestModule::
buildAndFillInBlockVector(Alien::VectorData &vectorB, ConstArrayView<Integer> allUIndex,
                          ConstArray2View<Integer> allPIndex, ConstArrayView<Integer> allXIndex ALIEN_UNUSED_PARAM,
                          ConstArrayView<Real> value) {
    Alien::LocalBlockVectorWriter b(std::move(vectorB));
    ENUMERATE_CELL(icell, m_areaT.own()) {
...
Alien::SolverStatus
AlienTestModule::solve(Alien::ILinearSolver *solver, Alien::MatrixData &matrixA, Alien::VectorData &vectorB,
                       Alien::VectorData &vectorX, AlienTestOptionTypes::eBuilderType builderType) {
