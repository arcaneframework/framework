...
void
IFPVector::
setValues(const double * values, Arccore::Integer const* node_list, Arccore::Integer const* node_to_local_node)
{
  _check();
  if(m_resizable_block_size)
    F2C(ifpsetrsvectordata)((double*)values,(Arccore::Integer*)node_list);
  else
    F2C(ifpsetvectordata)((double*)values,(Arccore::Integer*)node_list,(Arccore::Integer*) node_to_local_node);
...
_check() const
{
  if( !VectorInternal::isInstancied()) 
    throw Arccore::FatalErrorException(A_FUNCINFO,"Cannot use IFPVector before its graph initialization");
  if (m_internal==NULL) 
    throw Arccore::FatalErrorException(A_FUNCINFO,"Cannot use IFPVector before its own initialization");
}
