...
public :
  ...
  typedef MueLu::AMGXOperator<scalar_type,
                              local_ordinal_type,
                              global_ordinal_type,
                              node_type>                     amgx_prec_type ;
#endif
...
public :
  ...
  void
  initPrecondParameters (IOptionsTrilinosSolver* options,MPI_Comm const* comm)
  {
      ...
      case TrilinosOptionTypes::MueLuPC:
        if(options->muelu().size()>0)
        {
          ...
          if(options->muelu()[0]->amgx().size()>0)
          {
            if(options->muelu()[0]->amgx()[0]->enable())
            {
              ...
              if(options->muelu()[0]->amgx()[0]->config().size()>0)
              {
                ...
                for(auto const& param : options->muelu()[0]->amgx()[0]->config()[0]->parameter())
                {
                ...
                plist.set("json file",options->muelu()[0]->amgx()[0]->parameterFile().localstr()) ;
          ...
          else
          {
            ...
            {
              m_precond_parameters->set("repartition: enable",           options->muelu()[0]->repartition()[0]->enable()) ;
              m_precond_parameters->set("repartition: start level",      options->muelu()[0]->repartition()[0]->startLevel()) ;
              m_precond_parameters->set("repartition: min rows per proc",options->muelu()[0]->repartition()[0]->minRowsPerProc()) ;
...
#endif
      M = createPreconditioner(A, m_precond_name,m_precond_parameters, std::cout, std::cerr);
