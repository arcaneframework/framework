...
template <typename ValueT>
void
SimpleCSRMatrixMultT<ValueT>::mult(const VectorType& x, VectorType& y) const
{
...
template <typename ValueT>
void
SimpleCSRMatrixMultT<ValueT>::mult(const UniqueArray<Real>& x, UniqueArray<Real>& y) const
{
  if (m_matrix_impl.m_is_parallel)
    _parallelMult(x, y);
  else
...
template <typename ValueT>
void
SimpleCSRMatrixMultT<ValueT>::_parallelMult(
...
template <typename ValueT>
void
SimpleCSRMatrixMultT<ValueT>::_seqMult(const VectorType& x_impl, VectorType& y_impl) const
{
  Real* y_ptr = y_impl.getDataPtr();
  Real* x_ptr = (Real*)x_impl.getDataPtr();
  ConstArrayView<Real> matrix = m_matrix_impl.m_matrix.getValues();
...
template <typename ValueT>
void
SimpleCSRMatrixMultT<ValueT>::_seqMult(
    const UniqueArray<Real>& x_impl, UniqueArray<Real>& y_impl) const
{
  Real* y_ptr = dataPtr(y_impl);
  Real* x_ptr = (Real*)dataPtr(x_impl);
...
template <typename ValueT>
void
SimpleCSRMatrixMultT<ValueT>::_parallelMultBlock(const VectorType& x, VectorType& y) const
{
  Integer alloc_size = m_matrix_impl.m_local_size + m_matrix_impl.m_ghost_size;
...
template <typename ValueT>
void
SimpleCSRMatrixMultT<ValueT>::_seqMultBlock(const VectorType& x, VectorType& y) const
{
