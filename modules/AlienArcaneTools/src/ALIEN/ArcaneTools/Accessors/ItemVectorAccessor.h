...
class ItemVectorAccessorT
    : public Common::VectorWriterBaseT<ValueT>
{
public:
...
public:
  ...
  public:
   struct Generic
   {
     ...
     template <typename Op, typename VarT>
     static void process(VectorElement& v, const VarT& var, Arccore::Integer block_size);
     template <typename Op, typename VarT>
     static void process(VarT& var, const VectorElement& v, Arccore::Integer block_size);
   };
...
template <typename VarT, typename ValueT = Arccore::Real> class ScalarAnyItemVariableT
{
 private:
  typedef typename ItemVectorAccessorT<ValueT>::VectorElement::Generic Generic;
  friend struct ItemVectorAccessorT<ValueT>::VectorElement::
      Generic; // pour l'acc�s � m_var par process : a remplacer par des acces
  ...
  if (v.isVariableBlockSize()) {
    ...
    for (Arccore::Integer i = 0; i < nIndex; ++i)
      for (Arccore::Integer j = 0; j < block_size; ++j)
        Op::apply(
            values[block_size * (indices[i] - offset) + j], var.valueAt(localIds[i])[j]);
  }
}
    ...
    Arccore::Integer block_size)
{
  ...
  if (v.isVariableBlockSize()) {
    ...
    for (Arccore::Integer i = 0; i < nIndex; ++i)
      for (Arccore::Integer j = 0; j < block_size; ++j)
        Op::apply(
            var.valueAt(localIds[i])[j], values[block_size * (indices[i] - offset) + j]);
  }
}
...
process(typename ItemVectorAccessorT<ValueT>::VectorElement & v, const ScalarAnyItemVariableT<VarT, ValueT> & var)
{
  ...
  Arccore::ArrayView<Arccore::Real>& values = v.m_values; // donn�es du vecteur alg�brique
  ...
  for (Arccore::Integer igrp = 0; ((igrp < group_size) && (i < nIndex)); ++igrp) {
    ...
    { // Il y a du boulot... (sinon aucune donn�e associ�e � ce groupe, local ids trop loin)
      //ALIEN_ASSERT((ivar->isPartial() == family.isPartial(group)),("Inconsistent partial variable indicator"));
