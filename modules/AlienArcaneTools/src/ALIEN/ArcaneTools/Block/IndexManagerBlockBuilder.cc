...
namespace ArcaneTools {
  ...
  if(global_size != distribution.globalSize() ||
     local_size != distribution.localSize() ||
...
compute() const
{
  ...
  if(is_parallel)
  {
    ...
    parallel_mng->allGather(Arccore::ArrayView<Arccore::Integer>(1,&m_local_size),local_sizes);
  ...
  for(auto i = sendRequests.begin(); i != sendRequests.end(); ++i) {
    ...
    for (auto j = requests.begin(); j != requests.end(); ++j) {
      ...
      request.comm = new Arcane::SerializeMessage(parallel_mng->commRank(),
                                                  destDomainId,
                                                  Arcane::ISerializeMessage::MT_Send);
  ...
  for(Arccore::Integer isd=0, nsd=parallel_mng->commSize();isd<nsd;++isd) {
    ...
    while(recvCount-- > 0) {
      auto* recvMsg = new Arcane::SerializeMessage(parallel_mng->commRank(),
                                                   isd,
                                                   Arcane::ISerializeMessage::MT_Recv);
  ...
  for(auto i = recvRequests.begin(); i != recvRequests.end(); ++i) {
    ...
    {
      const Arccore::Integer dest = recvRequest.comm->destRank(); // Attention à l'ordre bizarre
      const Arccore::Integer orig = recvRequest.comm->origRank(); //       de SerializeMessage
  ...
  for(auto i = sendRequests.begin(); i != sendRequests.end(); ++i) {
    ...
    for(SendRequestByEntry::iterator j = requests.begin(); j != requests.end(); ++j) {
      ...
      auto* msg = new Arcane::SerializeMessage(parallel_mng->commRank(),
                                               destDomainId,
                                               Arcane::ISerializeMessage::MT_Recv);
...
#else
  throw Arccore::FatalErrorException(A_FUNCINFO, "Not yet implemented in External Mode");
#endif
  /*
  if(parallel_mng->commRank()==0)
  {
    ...
    for(ValuePerBlock::const_iterator it = m_ghost_sizes.begin(); it != m_ghost_sizes.end(); ++it)
    {
      std::cout << "index: " << it.key() << " value: " << it.value() << "\n";
    }
    std::cout << "m_all_offsets: \n";
    for(ValuePerBlock::const_iterator it = m_ghost_offsets.begin(); it != m_ghost_offsets.end(); ++it)
    {
      std::cout << "index: " << it.key() << " value: " << it.value() << "\n";
    }
  }
