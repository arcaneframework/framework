...
AnyItemAbstractFamily::clone() const
{
    ...
    Arccore::Int32ArrayView localIds, Arccore::Int64ConstArrayView uniqueIds) const
{
  ...
  Arccore::UniqueArray<Arccore::Int32> group_ids(
      size); // tableau mémorisant dans de quel groupe est issu l'item à la même position
  Arccore::UniqueArray<Arccore::UniqueArray<Arccore::Int64>> unique_id_by_groups(
      m_family.groupSize());
  for (Arccore::Integer i = 0; i < size;
       ++i) { // Classe les uids par leur groupe sous-jacent avant d'appeler les méthodes
              // par IItemFamily
    ...
    Arccore::Int32 localId) const
{
  ...
  return IAbstractFamily::Item(makeUID(igrp, item.uniqueId()),
      item.owner());
}
...
Arccore::SharedArray<Arccore::Int32> AnyItemAbstractFamily::
allLocalIds() const
{
