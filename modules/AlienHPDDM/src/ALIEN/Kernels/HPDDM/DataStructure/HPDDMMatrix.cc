...
template<>
int HPDDMInternal::getEnv(std::string const& key,int default_value)
{
...
_computeMPIGhostMatrix(typename MatrixInternal<ValueT>::CSRMatrixType const& A)
{
    ...
    for (std::size_t i = 0; i < num_neighbours_s; ++i)
    {
      ...
      for (int j = ids_offset_s[i]; j < ids_offset_s[i + 1]; ++j)
      {
        ...
        for (int k = kcol[lid]; k < kcol[lid+1]; ++k)
        {
          ...
          {
            if(col>=ids_offset_r[i_recv_neighb] && col<ids_offset_r[i_recv_neighb+1])
              ++nnz_ext_int_send[i];
            else
              ++nnz_ext_ext2_send[i];
          }
      ...
      send_request0[i] = Arccore::MessagePassing::mpSend(m_parallel_mng,ConstArrayView<int>(4, &count_send[4*i]), ranki, false);
    }
  }
  ...
  for (std::size_t i = 0; i < num_neighbours_r; ++i)
  {
    ...
    recv_request0[i] = Arccore::MessagePassing::mpReceive(m_parallel_mng,ArrayView<int>(4, &count_recv[4*i]), ranki, false);
  }
  ...
  UniqueArray<UniqueArray<ValueT >>         rbuffer_s(num_neighbours_s);
  {
    for (std::size_t i = 0; i < num_neighbours_s; ++i)
    {
      ...
      ibuffer_s[i].resize(3*(send_size+1)+nnz_ext_int_send[i]+nnz_ext_ext_send[i]+2*nnz_ext_ext2_send[i]) ;
      rbuffer_s[i].resize(nnz_ext_int_send[i]+nnz_ext_ext_send[i]+nnz_ext_ext2_send[i]+send_size) ;
        ...
        for (int k = kcol[lid];k <kcol[lid + 1]; ++k)
        {
          ...
          if(col<m_local_nrows)
          {
            ...
            else //if(dist_info.isInterfaceRow(col))
            {
              ...
              else
              {
                ...
                ibuffer_s[i][3*(send_size+1)+ext_ext_offset+ext_ext_ptr] = col_lid;
          ...
          else
          {
            if(col>=ids_offset_r[i_recv_neighb] && col<ids_offset_r[i_recv_neighb+1])
            {
              ...
              ibuffer_s[i][3*(send_size+1)+ext_int_offset+ext_int_ptr] = col -ids_offset_r[i_recv_neighb];
            ...
            else
            {
              ...
              //mpi_ext_ext2_cols_s[i][2*ext_ext2_ptr]   = recv_info.m_uids[col -ids_offset_r[0]] ;
              //mpi_ext_ext2_cols_s[i][2*ext_ext2_ptr+1] = recv_info.m_ranks[recv_neighb_id] ;
              //mpi_ext_ext2_values_s[i][ext_ext2_ptr]   = values[k];

              ibuffer_s[i][3*(send_size+1)+ext_ext2_offset+2*ext_ext2_ptr]   = recv_info.m_uids[col -ids_offset_r[0]];
              ibuffer_s[i][3*(send_size+1)+ext_ext2_offset+2*ext_ext2_ptr+1] = recv_info.m_ranks[recv_neighb_id];
      ...
      send_request0[i] = Arccore::MessagePassing::mpSend(m_parallel_mng,ibuffer_s[i],ranki, false);
      send_request1[i] = Arccore::MessagePassing::mpSend(m_parallel_mng,rbuffer_s[i],ranki, false);
  ...
  for (std::size_t i = 0; i < num_neighbours_r; ++i)
  {
    ...
    ibuffer_r[i].resize(3*(nrows[i]+1)+nnz_ext_int[i]+nnz_ext_ext[i]+2*nnz_ext_ext2[i]) ;
    rbuffer_r[i].resize(nnz_ext_int[i]+nnz_ext_ext[i]+nnz_ext_ext2[i]+nrows[i]) ;

    recv_request0[i] = Arccore::MessagePassing::mpReceive(m_parallel_mng,ibuffer_r[i], ranki,false);
    recv_request1[i] = Arccore::MessagePassing::mpReceive(m_parallel_mng,rbuffer_r[i], ranki,false);
  ...
  for (std::size_t i = 0; i < num_neighbours_r; ++i)
  {
    m_ghost_nnz += ibuffer_r[i][nrows[i]] + ibuffer_r[i][nrows[i]+1+nrows[i]] + ibuffer_r[i][2*(nrows[i]+1)+nrows[i]] ;
  }
...

  {
    ...
    for (std::size_t i = 0; i < num_neighbours_r; ++i)
    {
      ...
      ConstArrayView<Integer> mpi_ext_ext2_kcol(nrows[i]+1,&ibuffer_r[i][2*(nrows[i]+1)]) ;

      int mpi_ext_int_nnz = mpi_ext_int_kcol[nrows[i]] ;
      int mpi_ext_int_offset = 0 ;
      ConstArrayView<Integer> mpi_ext_int_cols(mpi_ext_int_nnz,&ibuffer_r[i][3*(nrows[i]+1)+mpi_ext_int_offset]) ;
      ConstArrayView<ValueT>  mpi_ext_int_values(mpi_ext_int_nnz,&rbuffer_r[i][mpi_ext_int_offset]) ;

      int mpi_ext_ext_nnz = mpi_ext_ext_kcol[nrows[i]] ;
      int mpi_ext_ext_offset = mpi_ext_int_offset+ mpi_ext_int_nnz;
      ConstArrayView<Integer> mpi_ext_ext_cols(mpi_ext_ext_nnz,&ibuffer_r[i][3*(nrows[i]+1)+mpi_ext_ext_offset]) ;
      ConstArrayView<ValueT>  mpi_ext_ext_values(mpi_ext_ext_nnz,&rbuffer_r[i][mpi_ext_ext_offset]) ;

      int mpi_ext_ext2_nnz = mpi_ext_ext2_kcol[nrows[i]] ;
      int mpi_ext_ext2_offset = mpi_ext_ext_offset+ mpi_ext_ext_nnz;
      ConstArrayView<Integer> mpi_ext_ext2_cols(2*mpi_ext_ext2_nnz,&ibuffer_r[i][3*(nrows[i]+1)+mpi_ext_ext2_offset]) ;
      ConstArrayView<ValueT>  mpi_ext_ext2_values(mpi_ext_ext2_nnz,&rbuffer_r[i][mpi_ext_ext2_offset]) ;

      int diag_correction_offset = mpi_ext_ext2_offset + mpi_ext_ext2_nnz ;
      ConstArrayView<ValueT> mpi_diag_correction(nrows[i],&rbuffer_r[i][diag_correction_offset]) ;
      ...
      for (int irow = 0; irow < nrows[i]; ++irow)
      {
          ...
          for (int k = mpi_ext_ext2_kcol[irow]; k < mpi_ext_ext2_kcol[irow + 1]; ++k)
          {
            ...
            int col_lid = neighb_id==-1 ? -1 : recv_info.getLocalIdFromUid(neighb_id,col_uid) ;
...
MatrixInternal<ValueT>::
_createDirchletMatrix(typename MatrixInternal<ValueT>::CSRMatrixType const& A)
{
...
HPDDM::MatrixCSR<ValueT>*
MatrixInternal<ValueT>::
_createNeunmanMatrix(typename MatrixInternal<ValueT>::CSRMatrixType const& A)
{
  auto& profile_neumann = m_csr_matrix_neumann.internal().getCSRProfile() ;
...
void
MatrixInternal<ValueT>::
_computeOverlapConnectivity(const CSRMatrixType& A)
{
  using namespace Arccore ;
  auto const& dist_info = A.getDistStructInfo() ;
  Integer domain_offset = A.getLocalOffset() ;
...
void
MatrixInternal<ValueT>::
_compute(HPDDM::MatrixCSR<ValueT>* matrix_dirichlet,
         HPDDM::MatrixCSR<ValueT>* matrix_neumann,
         unsigned short nu,
         bool schwarz_coarse_correction)
{
  ...
  {
    auto* pm = dynamic_cast<Arccore::MessagePassing::Mpi::MpiMessagePassingMng *>(m_parallel_mng);
    assert(pm) ;
    ...
    m_matrix.HPDDM::Subdomain<ValueT>::initialize(matrix_dirichlet,m_overlap, m_mapping,const_cast<MPI_Comm*>(comm));
