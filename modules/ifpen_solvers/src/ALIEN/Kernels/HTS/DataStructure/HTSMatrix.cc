...
template<>
int HTSInternal::getEnv(std::string const& key,int default_value)
{
...
void HTSInternal::initialize(Arccore::MessagePassing::IMessagePassingMng* parallel_mng)
{
  if(m_is_initialized)
    return ;
  ...
  AffinityMngType th_id_mng(*m_machine_info,set_aff,m_mpi_core_id_offset,-1,m_nb_hyper_threads);
  if(m_nb_threads>1)
    m_thread_env->setAffinity(th_id_mng);

  if(m_topology.get()==nullptr)
  {
    struct RunTimeSystem::NumaThreadComparator<ThreadEnvType> numa_comp(m_thread_env.get());
...
void HTSInternal::initMPIEnv(MPI_Comm comm)
{
...
bool
MatrixInternal<ValueT,is_mpi>::initMatrix(Arccore::MessagePassing::IMessagePassingMng* parallel_mng,
                                          int nrows,
                                          int const* kcol,
                                          int const* cols,
                                          int block_size)
...
MatrixInternal<ValueT,is_mpi>::computeDDMatrix()
{
  ...
  if(is_mpi && m_is_parallel)
  {
    ...
    if(m_dist_info.get()==nullptr)
    {
      ...
      if(m_is_parallel)
      {
        m_dist_info->compute( domain_offset,
                              mpi_env->getParallelMng(),
                              profile,
                              nullptr);
      }
      else
    ...
    if(m_partition.get()==nullptr)
    {
      ...
      m_partition.reset(partition_factory.create(local_profile,nrows,ghost_size,interface_filter,smetis,nb_subpart,true,nullptr)) ;
    ...
    {
      case 1:
        m_dd_matrix->setSendRecvPolicy(HartsSolver::CommProperty::Synch,HartsSolver::CommProperty::ASynch) ;
        break ;
      case 2:
        m_dd_matrix->setSendRecvPolicy(HartsSolver::CommProperty::ASynch,HartsSolver::CommProperty::Synch) ;
        break ;
      case 3:
        m_dd_matrix->setSendRecvPolicy(HartsSolver::CommProperty::ASynch,HartsSolver::CommProperty::ASynch) ;
        break ;
      default :
        m_dd_matrix->setSendRecvPolicy(HartsSolver::CommProperty::Synch,HartsSolver::CommProperty::Synch) ;
        break ;
    }
    ...
    if(m_dd_profile.get()==nullptr)
    {
      m_dd_matrix->initDD(*m_matrix,m_dist_info.get(),mpi_env->getParallelMng(),
                          m_partition_info.get(),m_partition.get(),
                          false,use_mpi_external_part,nullptr) ;
  ...
  else
  {
     if(m_partition.get()==nullptr)
     {
       ...
       m_partition.reset(partition_factory.create(m_matrix->getProfile(),smetis,nb_subpart,0,0,0,0,0,0,false)) ;
