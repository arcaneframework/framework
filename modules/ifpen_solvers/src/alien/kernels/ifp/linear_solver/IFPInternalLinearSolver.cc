...
IFPInternalLinearSolver::IFPInternalLinearSolver(
    Arccore::MessagePassing::IMessagePassingMng* parallel_mng, IOptionsIFPLinearSolver* options)
...
IFPInternalLinearSolver::init()
{
  ...
  alien_info(m_print_info, [&] {
      cout() << "IFPInternalLinearSolver::init";
  });
...
void
IFPInternalLinearSolver::updateParallelMng(Arccore::MessagePassing::IMessagePassingMng* pm)
{
  m_parallel_mng = pm;
}
...
IFPInternalLinearSolver::solve(const MatrixType& A, const VectorType& b, VectorSolType& x)
{
  ...
  if(!matrix.getSymmetricProfile() && m_options->ilu0Algo()==IFPSolverProperty::Normal)
    m_ilu0_algo=3;
  if(!matrix.getSymmetricProfile() && m_options->ilu0Algo()==IFPSolverProperty::Optimized)
    m_ilu0_algo=4;
  sentry.release() ;
...
ILinearSolver*
IFPInternalLinearSolverFactory(Arccore::MessagePassing::IMessagePassingMng* p_mng, IOptionsIFPLinearSolver* options)
{
