...
class Composite_to_MCG_VectorConverter : public Alien::IVectorConverter
{
 public:
 ...
 public:
  ...
  {
    return backendId<Alien::BackEnd::tag::mcgsolver>();
  }
...
void
Composite_to_MCG_VectorConverter::convert(
    const Alien::IVectorImpl* sourceImpl, Alien::IVectorImpl* targetImpl) const
{
  const auto& v = cast<Alien::CompositeKernel::Vector>(sourceImpl, sourceBackend());
  alien_debug([&] { cout() << "Converting CompositeVector: " << &v << " to MCGVector"; });
