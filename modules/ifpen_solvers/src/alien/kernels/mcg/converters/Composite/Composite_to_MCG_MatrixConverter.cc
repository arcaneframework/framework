    ...
    const SimpleCSRMatrix<Real>& sourceImpl, MCGMatrix& targetImpl) const
{
  ...
  if (not targetImpl.initMatrix(
          localSize, row_offset.unguardedBasePointer(), cols.unguardedBasePointer())) {
    throw FatalErrorException(A_FUNCINFO, "GPUSolver Initialisation failed");
  }

  const bool success =
      targetImpl.initMatrixValues(localSize, row_offset.unguardedBasePointer(),
          cols.unguardedBasePointer(), values.unguardedBasePointer());
    ...
    const SimpleCSRMatrix<Real>& sourceImpl, MCGMatrix& targetImpl) const
{
  ...
  if (not targetImpl.initSubMatrix01(nrows, dist.localRowSize(),
    ...
    const SimpleCSRMatrix<Real>& sourceImpl, MCGMatrix& targetImpl) const
{
  ...
  ConstArrayView<Integer> cols = profile.getCols();
  ConstArrayView<Real> values = matrixInternal.getValues();
  if (not targetImpl.initSubMatrix11(
          localSize, row_offset.unguardedBasePointer(), cols.unguardedBasePointer())) {
    throw FatalErrorException(A_FUNCINFO, "GPUSolver Initialisation failed");
  }
...
void
Composite_to_MCG_MatrixConverter::_buildBlock(
    const SimpleCSRMatrix<Real>& sourceImpl, MCGMatrix& targetImpl) const
{
  const CSRStructInfo& profile = sourceImpl.getCSRProfile();
  ...
  ConstArrayView<Real> values = matrixInternal.getValues();
  if (not targetImpl.initMatrix(
          local_size, row_offset.unguardedBasePointer(), cols.unguardedBasePointer())) {
    throw FatalErrorException(A_FUNCINFO, "GPUSolver Initialisation failed");
  }

  const bool success =
      targetImpl.initMatrixValues(local_size, row_offset.unguardedBasePointer(),
          cols.unguardedBasePointer(), values.unguardedBasePointer());
