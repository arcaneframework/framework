...
public:
  ...
  {
    std::cout<<"init MCGCompositeMatrix with m_domain_offset = dist.rowOffset()/m_equations_num "<<std::endl ;
  }
...
private :
  ...
  bool _initMatrix(const int i,const int j,
                  const int block_size,const int block_size2,
                  const int nrow,const int ncol,const int *row_offset,const int *cols);
  bool _initTransMatrix(const int i,const int j,
                  const int block_size,const int block_size2,
                  const int nrow,const int ncol,const int *row_offset,const int *cols);
