...
public:
  ...
  Alien::BackEndId targetBackend() const { return AlgebraTraits<BackEnd::tag::hts>::name(); }
    ...
    const IVectorImpl* sourceImpl, IVectorImpl* targetImpl) const
{
  ...
  HTSVector<double,true>& v2 = cast<HTSVector<double,true>>(targetImpl, targetBackend());
